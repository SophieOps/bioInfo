...
public class AlignmentAlgo
{
	...
	private void maxValueRow(Fragment f1, Fragment f2, int[][] matrix) {
		...
		//Regarde dans la derni�re ligne
		for(int j = 0; j < f2.getCode().length()+1; j++)
		{
	...
	private void maxValueColumn(Fragment f1, Fragment f2, int[][] matrix) {
		...
		//Regarde dans la derni�re colonne
		for(int i = 0; i < f1.getCode().length()+1; i++)
		{
	...
	private void manageInclusion(Fragment f1, Fragment f2) {
		...
		if(f1.getCode().length() <= f2.getCode().length())//f1 peut �tre inclu � f2
		{
		...
		else //f2 peut �tre inclu � f1
		{
			if(maxValueColumn >= maxValueRow)
			{
				...
				{
					inclusion = -1;
					/*System.out.println("f2 inclus � f1");
					System.out.println(alignmentList.get(0));
					System.out.println(alignmentList.get(1));*/
				}
			}
	...
	private void alignmentReconstructor(int i, int j, int savei, int savej, Fragment f1, Fragment f2,
		...
		//On recopie tout les caract�res avant i de f1 et on mets des " " pour f2
		for(int a = 0; a < i; a++)
		{
		...
		//On recopie tout les caract�res avant j de f2 et on mets des " " pour f1
		for(int b = 0; b < j; b++)
		{
		...
		//On ajoute l'alignement calcul� pr�c�demment
		...
		//On ajoute ce qui se trouve apr�s l'alignement pour f1
		for(int d = savei; d<f1.getCode().length(); d++)
		{
			reconstructorF1+=f1.getCode().charAt(d);
		}
		
		//On ajoute ce qui se trouve apr�s l'alignement pour f2
	...
	//le premier �l�ment de la liste contient l'alignement de f1 et le deuxi�me �l�ment l'alignement de f2

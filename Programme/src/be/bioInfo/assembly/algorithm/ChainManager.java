...
public class ChainManager 
{
	...
	public static void constructChain(ArrayList<Edge> edgeList, String numColl)
	{
		...
		do
		{
			//Si on rentre dans le if on est dans le dernier arc � traiter
			if(edgeList.isEmpty())
	...
	private static  void manageConstruction(ArrayList<short[]> values, Edge currentEdge, int currentSeqIndex) {
		...
		for (int j = 0; j < currentEdge.getAlignment().getCode1().length(); j++)
		{
			...
			//On rajoute une cellule pour savoir si on est en pr�sence d'un gap
			if(currentSeqIndex>=values.size())
			{
				values.add(new short[6]);
				short[] value = values.get(currentSeqIndex);
				addValue(value, currentEdge.getAlignment().getCode1().charAt(j), currentEdge.getAlignment().getCost());//Ajoute 1 � la cellule repr�sentant la j �me lettre de code1
		    }
			...
			if(currentEdge.getAlignment().getCode1().charAt(j) == Alignment.GAP && values.get(currentSeqIndex)[5] != 1)
			{
				...
				addValue(value, currentEdge.getAlignment().getCode1().charAt(j), currentEdge.getAlignment().getCost());//Ajoute 1 � la cellule repr�sentant la j �me lettre de code1
				addValue(value, currentEdge.getAlignment().getCode2().charAt(j), currentEdge.getAlignment().getCost());//Ajoute 1 � la cellule repr�sentant la j �me lettre de code2
				//Si on a un gap � la j �me position de code 2
		        if (currentEdge.getAlignment().getCode2().charAt(j) == Alignment.GAP) 
		        {
		        	values.get(currentSeqIndex)[5] = 1;//Sp�cifie qu'on est en pr�sence d'un gap
		        }
				values.add(currentSeqIndex, value);//on d�cale le tableau � la position currentSeqIndex vers la droite et on ins�re le nouveau tableau � la position currentSeqIndex

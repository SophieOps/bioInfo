		...
		do
		{
			...
			for (int j = 0; j < currentEdge.getAlignment().getCode1().length(); j++)
			{
				...
				if(values.get(currentSeqIndex)[5] != 1) 
				{
	                ...
	                addValue(value, currentEdge.getAlignment().getCode1().charAt(j));//Ajoute 1 � la cellule repr�sentant la j �me lettre de code1
	                addValue(value, currentEdge.getAlignment().getCode2().charAt(j));//Ajoute 1 � la cellule repr�sentant la j �me lettre de code2
			...
			//TTTTGGGGG****  3
			//*******GGCCCC  4
			//Permet de se positionner sur le d�but de l'alignement donc sur le premier T de 2
			//Ensuite on se positionnera sur le premier G de 4
			sequenceIndex+=currentEdge.getAlignment().getStartIndex();
			
			
			//Recherche de l'arc suivant � traiter
